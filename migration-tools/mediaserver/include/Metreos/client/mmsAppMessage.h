...
//
#ifndef MMS_APPMESSAGE_H
#define MMS_APPMESSAGE_H
...
#define MMS_MAX_XMLMESSAGESIZE 2048
#define MMS_MAXXMLTAGSIZE        64
#define MAXDIGITPATTERNSIZE      15
#define MMS_MAXXMLPARAMSIZE    2048
...
#define MMS_REASONCODE_NAME    "reasonCode"
...
#define MMS_RFC2833_SIGNAL_NAME "rfc2833Signal"
...
class MmsAppMessage 
{
  ...
  MmsAppMessage(char* utf8message);         // Incoming message

  MmsAppMessage();                          // Outgoing message
  ...
  void  terminateReturnMessage(const int rc=MMS_OMIT_RESULTCODE, const int reason=0);  
  void  terminateReturnMessage(void* id, const int rc=MMS_OMIT_RESULTCODE, const int reason=0);    
                                            // Make inbound message outbound
  int   makeTurnaroundMessage(int resultCode, int reason, void* clientID=NULL);
  ...
  void  putResultCode(int resultcode, int reasoncode);       

  void  putReasonCode(int reasoncode);      // Insert reason code xml

  void  putUserToken(int token);            // Insert user token xml
  ...
  virtual ~MmsAppMessage();

  protected:
                                            
  char   ubuf[MMS_MAX_XMLMESSAGESIZE];   // Ansi or UTF-8 character buffer         
  WCHAR  wbuf[MMS_MAX_XMLMESSAGESIZE];   // Unicode character buffer
  char*  pu, *pparams, *ptermcondvalue, *pattrvalue;
  LPWSTR pw;
  int    commandno, termcondno, ubuflen, paramlength, serverid, usertoken; 
  void*  hqDestination;                             
  ...
  MmsAppMessageX* sd;                       // Static data (parse tables)
