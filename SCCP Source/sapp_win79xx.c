...
#include "ccapi.h"
//#include "configmgr.h"
//#include "prot_configmgr.h" /* creates a bunch of errors */
int config_get_value(int id, void *buffer, int length);
int config_get_string(int id, char *buffer);
void sip_config_get_display_name(int line, char *buffer);
//ulong sip_config_get_proxy_ipaddr(int line);
//int sip_config_get_proxy_port(int line);
int getAdapterAddr(char* TempEthernet, unsigned char *raw_mac);

typedef void *t_SrvHandle;
int sip_dns_gethostbysrv(const char *domain,
                         unsigned long *ipaddr_ptr,
                         unsigned short *port,
                         t_SrvHandle *psrv_order);
void dns_free_call_order(t_SrvHandle srv_handle);
int dns_gethostbyname(const char *hname,
                      unsigned long *ipaddr_ptr,
                      int timeout,
                      int retries);

#define SAPP_CFGID_LINE1_PROTOCOL 76
#define SAPP_CFGID_CCM1_ADDRESS   82
#define SAPP_CFGID_CCM1_PORT      87

#ifdef SAPP_PLATFORM_WIN79XX
#include "mac.h"
#endif
#endif
...
#if (SAPP_APP == SAPP_APP_GSM)
#define SAPP_CAUSE_NORMAL CC_CAUSE_NORMAL
#define SAPP_CAUSE_ERROR  CC_CAUSE_ERROR
#endif
...
extern sapp_info_t sapp_infos[1];
...
extern sapp_states_e sapp_state;
extern gapi_causes_e sapp_reset_cause;
extern sapp_status_data_info_t sapp_session_data;
#endif
extern int sapp_debug;

//extern gapi_cmaddr_t sapp_cmaddr[2];
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_setup(CC_SRC_SIP, call_id, line, conninfo, (boolean)alert_info,
             alerting_ring, alerting_tone, media, redirect,
             (boolean)replaces);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_setup(call_id, line, conninfo);
#endif
}
...
static void sapp_app_offhook (int call_id, int line)
{
    ...
    sccptest_offhook(call_id, line, 0);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_setup_ack(CC_SRC_SIP, call_id, line, conninfo, media);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_setup_ack(call_id, line, cause);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_proceeding(CC_SRC_SIP, call_id, line, conninfo);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_proceeding(call_id, line);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_alerting(CC_SRC_SIP, call_id, line, conninfo, media, (boolean)inband);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_alerting(call_id, line);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_connected(CC_SRC_SIP, call_id, line, conninfo, media);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_connect(call_id, line);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_connected_ack(CC_SRC_SIP, call_id, line, conninfo, media);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_connect_ack(call_id, line);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_release(CC_SRC_SIP, call_id, line, cause, NULL);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_release(call_id, line);
#endif
}
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_release_complete(CC_SRC_SIP, call_id, line, cause);
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_release_complete(call_id, line);
#endif
}
...
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccptest_feature_req(call_id, line, feature);
#endif
}
                                        ...
                                        void *sapp_conninfo)
{
    ...
    /*
     * We can just point point to the strings because caller_id
     * uses the string library - which means the data is valid.
     */
    sccp_conninfo->calling_name   = (char *)(caller_id->calling_name);
    sccp_conninfo->calling_number = (char *)(caller_id->calling_number);
    sccp_conninfo->called_name    = (char *)(caller_id->called_name);
    sccp_conninfo->called_number  = (char *)(caller_id->called_number);
#endif
}

#if (SAPP_APP == SAPP_APP_GSM)
static int sapp_sccp_to_sapp_payload (int sccp_payload)
{
    int sapp_payload;
    ...
    switch (sccp_payload) {
    default:
    case (GAPI_PAYLOAD_TYPE_G711_ULAW_64K):
        sapp_payload = 0;
        break;
    }
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sapp_payload = sccp_payload;
#endif
...
#if (SAPP_APP == SAPP_APP_GSM)
static int sapp_sapp_to_sccp_payload (int sapp_payload)
{
    int sccp_payload;
    ...
    default:
    case (0):
        sccp_payload = GAPI_PAYLOAD_TYPE_G711_ULAW_64K;
        break;
    }
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    sccp_payload = sapp_payload;
#endif
...
{
#if (SAPP_APP == SAPP_APP_GSM)
    cc_sdp_t *sdp = sapp_media;
    int      i;
    
    memset(sdp, 0, sizeof(*sdp));
    sdp->remote_avt_payload_type = -1;
    for (i = 0; i < CC_MAX_MEDIA_TYPES; i++)  {
        sdp->remote_media_types[i] = -1;
    }

    if (sccp_media != NULL)  {
        sdp->remote_addr.addr = ntohl(sccp_media->sccp_media.addr);
        sdp->remote_addr.port = ntohl(sccp_media->sccp_media.port);        
        sdp->remote_media_types[0] = sapp_sccp_to_sapp_payload(sccp_media->sccp_media.payload_type);
    }
    else {
        /*
         ...
         */
        /*
         * GSM expects addr to be in little-endian.
         */
        sdp->remote_addr.addr = ntohl(sapp_local_addr());
        sdp->remote_addr.port = 0x4000; /* 16384 */        
        sdp->remote_media_types[0] = sapp_sccp_to_sapp_payload(GAPI_PAYLOAD_TYPE_G711_ULAW_64K);
    }
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
    gapi_media_t *media  = sapp_media;
...
static void sapp_sapp_to_sccp_media (gapi_media_t *sccp_media, void *sapp_media)
{
    ...
    if (sapp_media != NULL) {
        sccp_media->sccp_media.addr = htonl(sdp->local_addr.addr);
        sccp_media->sccp_media.port = htonl(sdp->local_addr.port);
        sccp_media->sccp_media.payload_type = sapp_sapp_to_sccp_payload(sdp->local_media_types[0]);
    }
    else {
...
#if (SAPP_APP == SAPP_APP_GSM)
        call->sapp_call_id = cc_get_new_call_id();
#endif
#if (SAPP_APP == SAPP_APP_SCCPTEST)
                ...
                void *media, int alert_info, int privacy, int flag)
{
    ...
    if (sinfo->sapp_state != SAPP_S_REGISTERED) {
        sapp_app_release_complete(call_id, line, SAPP_CAUSE_NORMAL);
        return (0);
    }
    ...
    call->sinfo->sapp_sccp_cbs->setup(call->sinfo->sapp_sccp_handle, GAPI_MSG_SETUP,
                         ...
                         &(call->sccp_media), alert_info, privacy, NULL);
#else /* passthru test */
                      ...
                      gapi_causes_e cause)
{
    ...
    call->sinfo->sapp_sccp_cbs->feature_req(call->sinfo->sapp_sccp_handle,
                               GAPI_MSG_FEATURE_REQ,
                               call->sccp_conn_id, call->sccp_line,
                               sapp_sapp_to_gapi_feature(feature),
                               NULL, cause);
...
int sapp_sapp_setup (gapi_handle_t handle, int msg_id, int conn_id, int line,
...
int sapp_sapp_offhook (gapi_handle_t handle, int msg_id, int conn_id, int line)
{
...
int sapp_sapp_setup_ack (gapi_handle_t handle, int msg_id, int conn_id, int line,
                         ...
                         gapi_causes_e cause, gapi_precedence_t *precedence)
{
    ...
    if (cause == GAPI_CAUSE_OK) {
        ...
        sapp_app_setup_ack(call->sapp_call_id, call->sapp_line,
                           SAPP_CAUSE_NORMAL,
...
int sapp_sapp_proceeding (gapi_handle_t handle, int msg_id, int conn_id, int line,
...
int sapp_sapp_alerting (gapi_handle_t handle, int msg_id, int conn_id, int line,
                        gapi_conninfo_t *conninfo, gapi_media_t *media,
                        int inband, gapi_precedence_t *precedence)
{
    char *fname = "sapp_alerting";
    sapp_call_t *call;
    sapp_info_t *sinfo = (sapp_info_t *)handle;
    ...
    sapp_app_alerting(call->sapp_call_id, call->sapp_line,
...
int sapp_sapp_connect (gapi_handle_t handle, int msg_id, int conn_id, int line,
                       gapi_conninfo_t *conninfo, gapi_media_t *media,
                       gapi_precedence_t *precedence)
{             
    char *fname = "sapp_connect";
    ...
    sapp_app_connect(call->sapp_call_id, call->sapp_line,
                     &(call->sapp_conninfo), &(call->sapp_media));        
...
int sapp_sapp_connect_ack (gapi_handle_t handle, int msg_id, int conn_id,
                           ...
                           gapi_precedence_t *precedence)
{
    ...
    sapp_app_connect_ack(call->sapp_call_id, call->sapp_line,
                         &(call->sapp_conninfo),
                         &(call->sapp_media));    
...
int sapp_sapp_disconnect (gapi_handle_t handle, int msg_id, int conn_id, int line,
                          gapi_causes_e cause, gapi_precedence_t *precedence)
{
...
int sapp_sapp_release (gapi_handle_t handle, int msg_id, int conn_id, int line,
                       gapi_causes_e cause, gapi_precedence_t *precedence)
{
    char *fname = "sapp_release";
    sapp_call_t *call;
...
int sapp_sapp_release_complete (gapi_handle_t handle, int msg_id, int conn_id,
                                ...
                                gapi_precedence_t *precedence)
{
    ...
    sapp_app_release_complete(call->sapp_call_id, call->sapp_line,
                              SAPP_CAUSE_NORMAL);    
...
int sapp_sapp_openrcv_req (gapi_handle_t handle, int msg_id, int conn_id,
...
int sapp_sapp_closercv (gapi_handle_t handle, int msg_id, int conn_id,
                        int line, gapi_media_t *media)
{
...
int sapp_sapp_startxmit (gapi_handle_t handle, int msg_id, int conn_id,
                         int line, gapi_media_t *media)
{
...
int sapp_sapp_stopxmit (gapi_handle_t handle, int msg_id, int conn_id,
                        int line, gapi_media_t *media)
{
...
int sapp_sapp_feature_req (gapi_handle_t handle, int msg_id, int conn_id,
                           ...
                           gapi_feature_data_u *data, gapi_causes_e cause)
{
    ...
    sapp_app_feature_req(call->sapp_call_id, call->sapp_line, feature);
...
int sapp_sapp_feature_res (gapi_handle_t handle, int msg_id, int conn_id,
                           int line, gapi_features_e feature,
                           gapi_feature_data_u *data, gapi_causes_e cause)
{
...
int sapp_sapp_serviceurlstat_res (void* handle, gapi_msgs_e msg_id, struct gapi_serviceurl_t_ *serviceurl)
{
                        ...
                        int line, gapi_conninfo_t *conninfo)
{
    ...
    if (call->gapi_waiting == 1)  {
        call->gapi_waiting = 0;
        sapp_app_setup(call->sapp_call_id, call->sapp_line,
                       &(call->sapp_conninfo), 0, 0, 0, &(call->sapp_media),
                       NULL, 0);
    }        
    else if (call->gapi_waiting == 2) {
        call->gapi_waiting = 0;

        sapp_app_proceeding(call->sapp_call_id, call->sapp_line,
                            &(call->sapp_conninfo));
    }
