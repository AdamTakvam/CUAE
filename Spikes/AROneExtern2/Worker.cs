...
namespace AROneExtern
{
	...
	public class Worker
	{
        ...
        public bool Generate(long deviceStart, int deviceCount, int userDn, ArrayList mces)
        {
                ...
                try
                {
                        ...
                        response.@return.device.lines.Items.Length > 0)
                    {
                        ...
                        try
                        {
                            ...
                            Console.WriteLine("Grabbed line {0}", dn);
                            receiverDns.Add(dn);
            ...
            foreach(string mceIp in mces)
            {
                ...
                for(int i = 0; i < receiverDns.Count; i++)
                {
                    ...
                    addDeviceSql.AddFieldValue("is_ip_phone", 1);
                    ...
                    addLineSql.AddFieldValue("directory_number", userDn++);
                    ...
                    // Add external number to user profile.
                    ...
                    externalNumberSql.AddFieldValue("call_attempt_timeout", 0);
                    externalNumberSql.AddFieldValue("delay_call_time", 0);
                    externalNumberSql.AddFieldValue("timeofday_enabled", 0);
