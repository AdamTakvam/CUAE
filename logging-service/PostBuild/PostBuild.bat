...
set ProjectRoot=%MetreosWorkspaceRoot%\logging-service
set PostBuildLog=%ProjectRoot%\postbuild\postbuild.txt
set DeployDir=%MetreosBuildRoot%\LoggingService
...
xcopy %ProjectRoot%\LogServerConsole\bin\%BuildTarget%\LogServerConsole.exe        %DeployDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServerConsole\bin\%BuildTarget%\LogServerConsole.exe.config %DeployDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServerService\bin\%BuildTarget%\LogServerService.exe        %DeployDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServerService\bin\%BuildTarget%\LogServerService.exe.config %DeployDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServer\bin\%BuildTarget%\Metreos.LogServer.dll              %DeployDir% /Y >> %PostBuildLog%

xcopy %ProjectRoot%\LogServerConsole\bin\%BuildTarget%\LogServerConsole.pdb        %SymbolDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServerService\bin\%BuildTarget%\LogServerService.pdb        %SymbolDir% /Y >> %PostBuildLog%
xcopy %ProjectRoot%\LogServer\bin\%BuildTarget%\Metreos.LogServer.pdb              %SymbolDir% /Y >> %PostBuildLog%
