...
namespace RemotePortForwarding
{
	...
	class SshSession
	{
		...
		private TElRemotePortForwardSSHTunnel m_tunnel;
		...
		{
			SshForwarding fwd = new SshForwarding(TunnelConnection, m_forwardToHost, m_forwardToPort);
			fwd.OnChange += new RemotePortForwarding.SshForwarding.ForwardingEvent(f_OnChange);
			fwd.OnClose += new RemotePortForwarding.SshForwarding.CloseEvent(f_OnClose);
			fwd.OnDataSend += new RemotePortForwarding.SshForwarding.DataSendEvent(f_OnDataSend);
			fwd.OnFinish += new RemotePortForwarding.SshForwarding.ForwardingEvent(f_OnFinish);
			fwd.OnLog += new RemotePortForwarding.SshForwarding.LogEvent(f_OnLog);
			OnConnectionAdd(this, fwd);
		...
		private void f_OnClose(object sender)
		{
			...
			{
				SshForwarding f = (SshForwarding)sender;
				if (f.InState == SshForwardingInState.Active) 
				{
	...
	class SshForwarding
	{
		...
		private string			m_tohost = "";
		private int				m_toport = 0;
		private int				m_received = 0;
		private int				m_sent = 0;
		object					m_channelLock = new object();
		object					m_socketLock = new object();

		public SshForwarding(TElSSHTunnelConnection conn, string destHost, int destPort) 
		{
			m_socket = null;
		...
		void ForwardingThreadFunc() 
		{
			...
			try 
			{
				IPEndPoint ep = new IPEndPoint(Dns.Resolve(m_tohost).AddressList[0], m_toport);
				m_socket.Connect(ep);			
			...
			while ((!m_error) && (!((m_inState == SshForwardingInState.Closed) && (m_outState == SshForwardingOutState.Closed)))) 
			{
				bool changed = false;
				// socket operations
				if ((m_socket.Connected) && (m_outState == SshForwardingOutState.Active))
				{
				...
				else if ((m_outState == SshForwardingOutState.Closing) && (!(m_socket.Connected)))
				{
                    m_outState = SshForwardingOutState.Closed;
					changed = true;
				}
