' SSH Remote Port Forwarding demo application
...
Public Class SshSession
    ...
    Dim m_tunnel As TElRemotePortForwardSSHTunnel
    ...
    Private Sub SetupComponents()
        ' setting up tunnels
        m_tunnel = New TElRemotePortForwardSSHTunnel
    ...
    Private Sub m_tunnel_OnClose(ByVal Sender As Object, ByVal TunnelConnection As TElSSHTunnelConnection)
        ...
        If Not (TunnelConnection.Data Is Nothing) Then
    ...
    Private Sub m_tunnel_OnOpen(ByVal Sender As Object, ByVal TunnelConnection As TElSSHTunnelConnection)
        Dim fwd As SshForwarding = New SshForwarding(TunnelConnection, m_forwardToHost, m_forwardToPort)
        AddHandler fwd.OnChange, AddressOf f_OnChange
        AddHandler fwd.OnClose, AddressOf f_OnClose
        AddHandler fwd.OnDataSend, AddressOf f_OnDataSend
        AddHandler fwd.OnFinish, AddressOf f_OnFinish
        AddHandler fwd.OnLog, AddressOf f_OnLog
        ConnectionAdd(fwd)
    ...
    Private Sub f_OnClose(ByVal sender As Object)
        ...
        Try
            Dim f As SshForwarding = CType(sender, SshForwarding)
            If f.InState = SshForwardingInState.Active Then
...
Public Class SshForwarding
    ...
    Private m_tohost As String = ""
    Private m_toport As Integer = 0
    ...
    Public Sub New(ByVal conn As TElSSHTunnelConnection, ByVal destHost As String, ByVal destPort As Integer)
        m_socket = Nothing
        m_sshConnection = conn
        SetupConnection()
        m_error = False
    ...
    Private Sub ForwardingThreadFunc()
        ...
        ' connecting to the remote side
        Log("Connecting to " + m_tohost + "...", False)
        m_socket = New Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp)
        Try
            Dim ep As New IPEndPoint(Dns.Resolve(m_tohost).AddressList(0), m_toport)
            m_socket.Connect(ep)
        ...
        While ((Not m_error) AndAlso (Not ((m_inState = SshForwardingInState.Closed) AndAlso (m_outState = SshForwardingOutState.Closed))))
            ...
            ' socket operations
            If ((m_socket.Connected) AndAlso (m_outState = SshForwardingOutState.Active)) Then
            ...
            ElseIf (((m_outState = SshForwardingOutState.Active)) AndAlso ((m_inState = SshForwardingInState.Closing) Or (m_inState = SshForwardingInState.Closed))) Then
                ...
                m_socket.Close()
                changed = True
            ElseIf ((m_outState = SshForwardingOutState.Closing) AndAlso (Not (m_socket.Connected))) Then
                m_outState = SshForwardingOutState.Closed
                changed = True
            End If
            If (changed) Then
    ...
    Public ReadOnly Property ToHost() As String
        Get
            Return m_tohost
        End Get
    End Property

    Public ReadOnly Property ToPort() As Integer
        Get
            Return m_toport
        End Get
    End Property
