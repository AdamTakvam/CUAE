...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Me.txtPassword.Location = New System.Drawing.Point(88, 58)
        ...
        Me.txtBoxCert.Size = New System.Drawing.Size(272, 20)
        ...
        'chkTLS1
        '
        Me.chkTLS1.Location = New System.Drawing.Point(8, 48)
        ...
        Me.chkSSL3.Location = New System.Drawing.Point(68, 16)
        ...
        'chkSSL2
        '
        Me.chkSSL2.Location = New System.Drawing.Point(8, 16)
        ...
        'chkTLS11
        '
        Me.chkTLS11.Location = New System.Drawing.Point(68, 48)
        Me.chkTLS11.Name = "chkTLS11"
        Me.chkTLS11.Size = New System.Drawing.Size(64, 24)
...
#End Region
	...
	Private WithEvents SecureClient As SBSimpleSSL.TElSimpleSSLClient
	...
	Private sendCert As Boolean

	Private cert As SBX509.TElX509Certificate = New SBX509.TElX509Certificate

    Private Sub Init()
        inBuffer = New Byte(999) {}
    ...
    Private Sub cmdCert_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdCert.Click
		...
		openFileDialog1.Filter = "PFX-encoded certificate (*.pfx, *.p12)|*.PFX,*.p12"
	...
	Private Sub cmdBlock_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdBlock.Click
		...
		SecureClient.Address = comboBox1.Text
		SecureClient.Port = 443
		DoRequest()
	End Sub
	...
	Private Sub LoadCert()
		...
		If txtBoxCert.TextLength > 0 Then
			Try
				...
				cert = New SBX509.TElX509Certificate
				If cert.LoadFromStreamPFX(fs, txtPassword.Text, 0) = 0 Then
					sendCert = True
				End If
				fs.Close()
			Catch e As Exception
				sendCert = False
			End Try
			If Not sendCert Then
				MessageBox.Show("Failed to load certificate", strMsgBoxTitle, MessageBoxButtons.OK, MessageBoxIcon.Error)
				cert = Nothing
			End If
		End If
	End Sub
	...
	Private Sub DoRequest()
		...
		If SecureClient.Active Then
			...
			SecureClient.SendData(b, b.Length)
		End If
		On Error GoTo quit
		Dim Buffer As Byte()
		...
		While SecureClient.Active
			ReDim Buffer(16384)
			ToRead = 16384
			SecureClient.ReceiveData(Buffer, ToRead)
			s = Encoding.Default.GetString(Buffer)
			txtMemo1.Text = txtMemo1.Text + s
		End While
quit:
		Disconnect()
	End Sub
	...
	Private Sub SecureClient_OnCertificateNeededEx(ByVal Sender As Object, ByRef Certificate As SBX509.TElX509Certificate) Handles SecureClient.OnCertificateNeededEx
		If sendCert Then
			Certificate = cert
			sendCert = false
		Else
