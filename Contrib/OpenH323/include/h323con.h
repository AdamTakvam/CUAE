...
namespace OpalOSP {
  class Transaction;
  class Transaction::DestinationInfo;
};
*/
#endif
...
#ifdef DEFER_OLC
PLIST(H323OpenLogicalChannelList, H245_OpenLogicalChannel);
#endif
...
#ifdef DEFER_OLC
    /**Add copy of deferred OpenLogicalChannel PDU so we can send response
       later. Thread safe.
      */
    void AddDeferredOpenLogicalChannel(const H245_OpenLogicalChannel & pdu);

    /**Remove OpenLogicalChannel PDU (by session id) that we set
       aside so we could defer sending response; if not found, return NULL.
       Thread safe.
      */
    const H245_OpenLogicalChannel * RemoveDeferredOpenLogicalChannel(unsigned sessionID);

    /**Get the mutex that provides thread safety for directly accessing the
       list of deferred OLC(s).
     */
    const PMutex & GetDeferredOpenLogicalChannelsMutex() const
    { return deferredOpenLogicalChannelsMutex; }

    /**Get the set of incoming OpenLogicalChannel components whose processing
       has (and therefore responses have) been deferred. You probably need the
       corresponding mutex (use GetDeferredOpenLogicalChannelsMutex()) to
       provide thread safety while accessing this set of deferred OLCs.
     */
    const H323OpenLogicalChannelList & GetDeferredOpenLogicalChannels() const
    { return deferredOpenLogicalChannels; }
#endif
...
#ifdef DEFER_OLC
    // Used accessing deferred OLC(s). Don't know if needed, but to be safe...
    PMutex deferredOpenLogicalChannelsMutex;

    // List of deferred OLCs.
    H323OpenLogicalChannelList deferredOpenLogicalChannels;
#endif
...
#ifdef H323_TRANSNEXUS_OSP
  public:
    OpalOSP::Transaction * GetOSPTransaction() const
    { return ospTransaction; }

    void SetOSPTransaction(OpalOSP::Transaction * tran)
    { ospTransaction = tran; }

  protected:
    OpalOSP::Transaction * ospTransaction;
    virtual void CleanUpOSP();
    BOOL AuthoriseOSPTransaction(OpalOSP::Transaction & transaction, OpalOSP::Transaction::DestinationInfo & destInfo);
#endif

#ifdef DEFER_OLC
    /**Remove OpenLogicalChannel PDU (by session id) that we set
       aside so we could defer sending response; if not found, return NULL.
       Not thread safe. Used internally to avoid need for recursive mutex.
      */
    const H245_OpenLogicalChannel * UnsafeRemoveDeferredOpenLogicalChannel(unsigned sessionID);
#endif
