...
#include "SWItrcEventListener.h"

/*****************************************************************************
 *****************************************************************************
 *
 * API definition for SMARTRecognizer
 *
 *****************************************************************************
 ****************************************************************************/
 ...
 *  SWIrec_ERROR_SERVER_UNAVAILABLE no servers could be connected to
 *  SWIrec_ERROR_SERVER_CONNECTION_FAILED hard TCP/IP fault for existing
     connection
 *  SWIrec_ERROR_SERVER_NO_CHANNELS_AVAILABLE max channels reached on server
 *  SWIrec_ERROR_SERVER_NOT_ALLOCATED no server allocated to recognizer (only
     when using explicit server allocation)
 *  SWIrec_ERROR_SERVER_ALLOCATED tried to allocate a server to recognizer
     object but it already has a server allocated to it
 *  SWIrec_ERROR_SERVER_FREED tried to free a server from recognizer object
     but no server is currently allocated to it
...
typedef enum {
   ...
   * SWIrec_SYSTEM_ERROR 
   *   if SWIrecInit hasn't been called or if some non-memory-allocation OS request fails
   * SWIrec_INVALID_PARAMETER_VALUE if any required parameters are NULL or zero-length strings, etc
 ...
 * @return SWIrec_ERROR_SERVER_UNAVAILABLE
 *         if no servers could be connected to
 * @return SWIrec_ERROR_SERVER_NO_CHANNELS_AVAILABLE
 *         server available, but max channels reached on server
 * @return SWIrec_SUCCESS on success
 */
 ...
 *
 * @param rec Recognizer handle
 * @param resource_type name of resource
 * @param resource_value value of resource
 *
 * @return SWIrec_ERROR_NO_SESSION_NAME if recognizer doesn't have a session name
 * @return SWIrec_ERROR_BUSY
 *         if called while recognition is active.
 * @return SWIrec_ERROR_UNSUPPORTED 
 *         lic mode is not explicit (SWIrec_LICENSE request)
 * @return SWIrec_ERROR_LICENSE_ALLOCATED
 *         if license was already allocated (SWIrec_LICENSE request)
 * @return SWIrec_ERROR_NO_LICENSE if license checkout request fails (SWIrec_LICENSE request)
 * @return SWIrec_ERROR_INVALID_LANGUAGE if language is invalid (SWIrec_LANGUAGE request)
 * @return SWIrec_ERROR_INVALID_MEDIA_TYPE if audio-media type is invalid (SWIrec_AUDIO reques)
 * @return SWIrec_INVALID_PARAMETER 
 *         if resource_type or resource_value isn't recognized
 */
SWIrecFuncResult ALTAPI SWIrecResourceAllocate(SWIrecRecognizer *rec,
 ...
 * @return SWIrec_ERROR_UNSUPPORTED 
 *         lic mode is not explicit (SWIrec_LICENSE request)
 * @return SWIrec_ERROR_LICENSE_FREED
 *         if license was already freed (SWIrec_LICENSE request)
 * @return SWIrec_ERROR_NO_LICENSE if license checkout request fails (SWIrec_LICENSE request)
 * @return SWIrec_ERROR if resource was not already allocated
 * @return SWIrec_INVALID_PARAMETER 
 *         if resource_type is invalid
 * @return SWIrec_INVALID_PARAMETER_VALUE
 *         if resource_value is invalid
 */
SWIrecFuncResult ALTAPI SWIrecResourceFree(SWIrecRecognizer *rec,
					   const wchar_t *resource_type,
					   wchar_t *resource_value);
...
typedef struct SWIrecGrammarData
{
   ...
   *        type string/2.0: "text/xml", "application/srgs+xml", "application/x-swi-grammar"
 ...
 * immediately when SWIrecGrammarFree() is called
 */
#define SWIREC_MEM_FLUSH_ON_FREE L"swirec_mem_flush_on_free"


/**
 * Property to indicate what language to use for the grammar
 */
 ...
 *         Grammar activation may cause a grammar load so all those errors may be 
 *         returned.
 */
...
/**
 * Set names for the current session or phone call.
 * These names are used for logging, LEARN, and tuning tools.
 * This function is typically called at the beginning of a call.
 *
 * @param rec Recognizer handle
 * @param channelName Name of channel
 * @param applicationName Name of application
 *                        This should be unique across different applications
 *                        in order for tools to distinguish between grammars
 *                        that may have the same name across applications.
 *
 * @return SWIrec_SUCCESS on success
 * @return SWIrec_ERROR_BUSY if
 *         called while recognition is active.
 * @return SWIrec_ERROR_INVALID_PARAMETER if
 *         either channelName or applicationName are NULL.
 */
SWIrecFuncResult ALTAPI SWIrecRecognizerSetSessionName(SWIrecRecognizer *rec,
 ...
 *         c. invalid combination of samples->len, samples->samples, and audio type
 * @return SWIrec_ERROR_NO_SESSION_NAME if recognizer doesn't have a session name
...
/**
 * Retrieve number of nbest answers in current results
 *
 * @param resultData Pointer to results data structure
 * @param numAnswers Returned number of answers
 *
 * @return SWIrec_SUCCESS on success
 */
SWIrecFuncResult ALTAPI SWIrecGetNumAnswers(SWIrecResultData *resultData,
                                     unsigned int *numAnswers);


/**
 * Retrieve list of keys from current results
 * The list is valid until the next call to RecognizerStart(),
 * RecognizerCompute(), ParseDTMFResults(), or GetKeyList().
 *
 * @param resultData Pointer to results data structure
 * @param nbestIndex Get keys for the nth answer.
                     If this is out of range, then
*                    SWIrec_ERROR_INVALID_NBEST_INDEX is set
 * @param keys Array of keys where result is returned, allocated by the
 *             recognizer
 * @param numKeys Number of keys in array
 *
 * @return SWIrec_SUCCESS on success
 * @return SWIrec_ERROR_INVALID_NBEST_INDEX
 *         if nbestIndex is out of range
 */
SWIrecFuncResult ALTAPI SWIrecGetKeyList(SWIrecResultData *resultData,
                                  unsigned int nbestIndex,
                                  wchar_t ***keys,
                                  unsigned int *numKeys);


/**
 * Query results for a specific key/value pair
 *
 * @param resultData Pointer to results data structure
 * @param nbestIndex Get value for the nth answer.  If nbestIndex is out of
 *                   range, then SWIrec_ERROR_INVALID_NBEST_INDEX is set
 * @param key Result key to retrieve
 *            If key is NULL, value and score are returned for the entire
 *            answer (identical to using key SWI_meaning)
 *            If key is not valid, then the returned value is an empty string,
 *              and the returned score is 0 (the function returns SWIrec_SUCCESS)
 *
 * @param value Returned value for key within results data.  The value string
 *            is allocated by the recognizer.
 * @param score Returned confidence score for key
 *
 * @return SWIrec_SUCCESS on success
 * @return SWIrec_ERROR_INVALID_NBEST_INDEX
 *         if nbestIndex is out of range
 */
SWIrecFuncResult ALTAPI SWIrecGetKeyValue(SWIrecResultData *resultData,
                                   unsigned int nbestIndex,
                                   const wchar_t *key,
                                   wchar_t **value,
                                   int *score);
...
/**
 * Return an XML result without having to call SWIrecGetKeyList and
 * SWIrecGetKeyValue
 *
 ...
 *              This value is recommended to be composed of a
 *              token/value list which matches the recognizer logging format.
 *
 * @return SWIrec_SUCCESS on success
 */
 ...
 *
 * @return SWIrec_SUCCESS on success
 * @return SWIrec_ERROR_INACTIVE if recognition
 *         is not active.
 */
...
/**
 * Clean up memory allocation for thread.
 * Releases any thread-specific storage.  Only needed in client/server
 * environments.
 */
