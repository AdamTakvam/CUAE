 ...
 ************************************************************************
 *
 * $Id: VXIinet.h,v 1.42 2003/10/24 19:26:09 dsmith Exp $
 *
 ************************************************************************
 */
 ...
 * @memo Keys identifying properties in VXIMap for Prefetch( ) and Open( )
...
/*@{*/
	...
	cached object no older than this value (given in seconds). A
...
#define INET_URL_QUERY_ARGS     L"inet.urlQueryArgs"
  ...
  /**
   * This property specifies whether the connection should be closed after the
   * fetch is completed.  This property, if specified and is a non-zero
   * VXIInteger, causes the the connection to be closed after the fetch.  If
   * specified and is zero VXIInteger, then an attempt to keep the connection
   * opened will be made.  If not specified, or not VXIInteger, then behaviour
   * is implementation dependant and should be specified in the
   * initialization of the inet layer
   */
#define INET_CLOSE_CONNECTION   L"inet.closeConnection"

  /**
   * This property specifies whether a new connection should be established
   * regardless of the state of a previously established connection.  If
   * specified and is a non-zero VXIinteger, Open() will close any existing
   * connection and attempt establishing a new connection.  If specified and
   * is a zero VXIInteger, then Open() will try to reuse the existing
   * connection.  If not specified, or not VXIInteger, then behaviour
   * is implementation dependant and should be specified in the
   * initialization of the inet layer
   *
   * Whether the connection (whether new of existing) is closed or not after
   * the fetch is still controlled by the INET_CLOSE_CONNECTION property.
   *
   **/
...
#define INET_INFO_VALIDATOR      L"inet.info.validator"
  ...
  /** HTTP status code, always returned for successful HTTP Open( )
    operations, or HTTP Open( ) operations that fail due to the remote
    web server returning a HTTP failure code in the HTTP response.
    Never returned in other cases (such as for failures for file://
    fetches, or failures for HTTP fetches due to non-HTTP response
    failures such as TCP/IP connection failures).  HTTP status code
    from the last HTTP response received related to the Open( )
    request.  Returned as a VXIInteger.  For example, a successful
    Open( ) for a http:// URL will most commonly report a HTTP status
    200 (OK), while a failed attempt will often report a HTTP status
    404 (Not Found).  Note that interim response codes, such as HTTP
    status 301 (Redirect -- Moved Permanently) should not be returned
    as the VXIinet implementation should respond by doing a HTTP
    request against the redirected location, resulting in a new HTTP
    status code from the new HTTP response. */
#define INET_INFO_HTTP_STATUS    L"inet.info.httpStatus"


  /**
   * INET_INFO_VALIDATOR_STRONG. Open() must return this property as a
   * VXIinteger with value TRUE when a strong validator is available for the
   * page.
   **/
#define INET_INFO_VALIDATOR_STRONG L"inet.info.validatorStrong"

  /**
   * Expires hint.  Timestamp indicating when the cache entry will expire.
   * The property is returned by Open() as a VXIInteger representing an
   * ANSI/ISO time_t integer, representing the number of seconds since Jan 1,
   * 1970). If the entry is already expired, a VXIInteger of value 0 is
   * returned.  This number only indicates this is the time where the request
   * has to be made again to the remote web server. This doesn't necessarily
   * mean that there will be new data after this time (For example, having the
   * property INET_INFO_VALIDATOR_STRONG with
   * an EXPIRES_HINT of 0, means the URL has to
   * be revalidated but does not imply that the data will change.
   **/
...
typedef struct VXIinetInterface {
   ...
   *
   * @param stream       [IN/OUT] Stream to close
   *
   * @return VXIinet_RESULT_SUCCESS on success
   */
   ...
   *
   * NOTE: VXIinet_RESULT_END_OF_STREAM may be returned even when
   *   nread is greater then 0.  Example: if there are 20 more bytes
   *   remaining in a stream and the client component requests 50
   *   bytes via Read( ), the implementation should return
   *   VXIinet_RESULT_END_OF_STREAM with nread = 20. If the client
   *   component then (incorrectly) requests 50 more bytes via Read( ),
   *   return VXIinet_RESULT_END_OF_STREAM again with nread = 0.
   *
