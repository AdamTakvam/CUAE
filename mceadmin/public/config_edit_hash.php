...
{
    $values = $_POST['values'];
    if (!empty($_POST['add_v_key_column']) && !empty($_POST['add_v_value']))
    {
        $values[] = array('key_column' => $_POST['add_v_key_column'],
                          'value'      => $_POST['add_v_value']);
        $response = "Key and value pair added";
    }
    else
    {
        $errors->Add("Blank keys and/or values are not allowed.");
    }
}
else if ($_POST['delete'])
{
    ...
    $response = "Key and value pair deleted";
}
else
{
...
$page->Display('config_edit_hash.tpl');
