...
/* This page allows the user to create CTI Device Pools for a CallManager cluster.  
 * A CTI Device Pool cannot be created unless a CTI Manager exists for the 
 * associated CallManager cluster.  A CTI Device Pool creates references to a series
 * of devices based on their device names.
 */
...
$conf_metas = ComponentUtils::get_standard_config_metas(ComponentType::CTI_DEVICE_POOL);
...
if ($_POST['submit'])
{
    ...
    if ($errors->IsEmpty())
    {
        ...
        $db->Execute("INSERT INTO mce_components (name, type, status, version, description) " .
                     "VALUES (?,?,?,?,?)",
                     array($_POST['name'], ComponentType::CTI_DEVICE_POOL, ComponentStatus::ENABLED_RUNNING, '1.0', $_POST['description']));
        $dp_id = $db->Insert_ID();
        ...
        EventLog::log(LogMessageType::AUDIT, 'CTI Device Pool Created', LogMessageId::IPT_SERVER_CREATED, print_r($log_values, TRUE));
        ...
        Utils::redirect("manage_device_pool.php?id=$dp_id&type=" . ComponentType::CTI_DEVICE_POOL);
    }
}
...
$page = new Layout();
$page->SetPageTitle("Create A CTI Device Pool");
...
$page->Display('add_cti_device_pool.tpl');
