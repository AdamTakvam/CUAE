...
require_once("class.SipDomainMember.php");

class SipDevicePool extends SipDomainMember
{
        ...
        if ($this->mErrorHandler->IsEmpty())
        {
            ...
            if ($count == 0)
            {
                $this->mDb->Execute("INSERT INTO mce_call_manager_devices (device_name, device_type, status, mce_components_id) VALUES (?,?,?,?)",
                             array($deviceName, DeviceType::SIP_DEVICE, DeviceStatus::ENABLED_STOPPED, $this->mId));
                $this->Refresh();
                EventLog::log(LogMessageType::AUDIT, 'SIP Device Pool contents changed', LogMessageId::IPT_DEVICE_POOL_CHANGED, print_r($_POST, TRUE));
                return TRUE;
            }
            else
    ...
    public function BulkAddDevices($startMac, $count, $exceptions = array())
    {
        if ($this->mErrorHandler->IsEmpty())
        {
            ...
            {
                $this->mDb->Execute("INSERT INTO mce_call_manager_devices (device_name, device_type, status, mce_components_id) VALUES (?,?,?,?)",
                            array($device, DeviceType::SIP_DEVICE, DeviceStatus::ENABLED_STOPPED, $this->mId));
            }

            $log_data = $_POST;
            $log_data['created_devices'] = $create_devices;
            EventLog::log(LogMessageType::AUDIT, 'SIP Device Pool contents changed', LogMessageId::IPT_DEVICE_POOL_CHANGED, print_r($log_data, TRUE));
