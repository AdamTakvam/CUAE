...
class ReplicationSettingsManagement
{
    ...
    {
        if (!file_exists(METREOS_ROOT . ReplicationConfig::MYSQL_REPLICATION_SETTINGS))
            return TRUE;
    
        // Parse XML settings file into a set of arrays
		$settings = array();
		$settings_xml = file_get_contents(METREOS_ROOT . ReplicationConfig::MYSQL_REPLICATION_SETTINGS);
		if (!$settings_xml)
    ...
    public function WriteSettingsFile()
    {
        ...
        file_put_contents(METREOS_ROOT . ReplicationConfig::MYSQL_REPLICATION_SETTINGS, implode("\n",$out));
    }
