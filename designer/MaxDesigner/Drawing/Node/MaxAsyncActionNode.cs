...
namespace Metreos.Max.Drawing
{
    ...
    {      
        ...
        {                                         
            ...
            bool isLegacyApp = Const.IsPriorVersion08(MaxProjectSerializer.serializedVersionF);
                       
            foreach(MaxAppTreeNode node in treenodes)  // Treenode hosts its actions
            {
                ...
                if (evhnode != null)  
                    ...
                    else evhnode.AddReference(this); 
            }
        } 
        ...
        public override bool CanChangeText(string oldname, string newname, object tag)
        {
            ...
            switch(GetTextChangedState(oldname, newname))
            {
                ...
                case ChangeStates.OldSingleNewExist:
                    ...
                    if (this.VerifyMatchingEvent(oldname, newname, tag, false))        
        ...
        public override void OnTextChanged(string oldname, string newname, object tag)
        {
            ...
            MaxManager.Instance.AppTree().ChangeHandler(this, treenode, oldname, newname); 
        }
        ...
        public bool DisposePriorReference(string oldfname, object tag)
        {
            ...
            bool deleteOK = false;
            int referrersCount = 0;
             
            if (Const.IsPriorVersion08(MaxProjectSerializer.serializedVersionF))
            { 
                referrersCount = app.GetAsyncActionsReferringTo(oldfname).Count
            ...
            else                                   
            {
                ...
                MaxAppTreeNodeFunc treenodeFun  
                    = appTree.GetFirstEntryFor(oldfname) as MaxAppTreeNodeFunc;
         
                if (treenodeFun != null)
                    referrersCount = treenodeFun.CanvasNodeCallActions.Count;
                if (treenodeFun is MaxAppTreeNodeEVxEH)
                    referrersCount += (treenodeFun as MaxAppTreeNodeEVxEH).References.Count;

                if (referrersCount > 1) 
                {
                    ShowCannotRenameReferencedFunctionMsg();
                    return false;
                }
            }
            ...
            if  (!oldfcanvas.CanDelete()) 
                 InformHandlerCannotDelete(this.NodeName, oldfname, null);
            else
        ...
        private bool VerifyMatchingEvent(string oldname, string newname, object tag, bool notify)
        {
            ...
            if (notify) Utl.ShowCannotRenameNodeDialog(true);
            return false;
        }


        public static bool ShowCannotRenameReferencedFunctionMsg()
        {
            MessageBox.Show(MaxManager.Instance, Const.CantRenameReferencedFunctionMsg,
                Const.RenameFunctionDlgTitle, MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return false;
        }
