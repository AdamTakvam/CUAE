/**
 * $Id: MtSipStackRuntime.cpp 16468 2005-11-30 17:28:14Z jdliau $
 */
#include "stdafx.h"
#include <sstream>
#include <Winsock2.h>
...
SdpContents::Session::Medium MtSipStackRuntime::DefaultMedium("audio", 0/*txPort*/, 0, "RTP/AVP");
const char* MtSipStackRuntime::DefaultRealm= "";
const char* MtSipStackRuntime::TelephoneEventParam = "0-15";
const char* MtSipStackRuntime::BitBucketIp = "1.2.3.4";
const int MtSipStackRuntime::BitBucketPort = 1234;

MtSipStackRuntime::MtSipStackRuntime() :
	...
	m_devicePortMapMutex(),
	m_minPortMutex(),
	m_phonePort(SIP_SERVICE_PORT),
	m_trunkPort(SIP_SERVICE_PORT),
	m_telephoneEventCodec("telephone-event", 101, 8000),
	m_clientSH(this),
	m_logTimingStat(false)
{
	...
	m_telephoneEventCodec.parameters().append(TelephoneEventParam, strlen(TelephoneEventParam));
}

void MtSipStackRuntime::WriteToIpc(const int messageType, Metreos::FlatMapWriter& flatmap)
{
	SIPLOG((Log_Verbose, "WriteToIpc: Type=%d", messageType));
    m_ipcServer.Write(messageType, flatmap, m_sessionId);
}

int MtSipStackRuntime::AddMessage(Sip::MtSipMessage* msg)
{
...
int MtSipStackRuntime::PostStartupMsg()
{
	         resip::FdSet fdset;

	MtSipMessage* msg = new MtSipMessage(Msgs::Start);
...
int MtSipStackRuntime::PostShutdownMsg()
{
	MtSipMessage* msg = new MtSipMessage(Msgs::Stop);
...
void MtSipStackRuntime::PostStartStackMsg(MtSipMessage *pMsg)
{
	if (pMsg == NULL)
		pMsg = new MtSipMessage(Msgs::StartStack);

    AddMessage(pMsg);
}

void MtSipStackRuntime::PostStopStackMsg()
{
    MtSipMessage* msg = new MtSipMessage(Msgs::StopStack);
    AddMessage(msg);
}

int MtSipStackRuntime::svc(void)
{
	         resip::FdSet fdset;
    ...
    {        
        ...
        MtSipMessage* msg = static_cast<MtSipMessage*>(amsg);
        ACE_ASSERT(msg != 0);
...
void MtSipStackRuntime::OnStart(Sip::MtSipMessage& message)
{
	m_runtimeStartedMutex.acquire();
...
void MtSipStackRuntime::OnStop(Sip::MtSipMessage& message)
{
	m_runtimeStartedMutex.acquire();
	if (m_started)
	{
...
void MtSipStackRuntime::ClearDevicePortMap()
{
	//clear deviceportmap
...
void MtSipStackRuntime::StartStack()
{
	OnStartStack(MtSipMessage());
}


void MtSipStackRuntime::OnStartStack(Sip::MtSipMessage& message)
{
...
void MtSipStackRuntime::OnParameterChanged(MtSipMessage& message)
{
	...
	MtSipMessage *pMsg = new MtSipMessage(Msgs::StartStack, pdata, message.length());
...
void MtSipStackRuntime::OnRegisterDevices(MtSipMessage& message)
{
	RegisterDevices(message, NULL);
}

void MtSipStackRuntime::OnUnregisterDevices(MtSipMessage& message)
{
	int expires = 0;
...
void MtSipStackRuntime::RegisterDevice(SharedPtr<UserProfile> up, 
...
void MtSipStackRuntime::RegisterDevices(Sip::MtSipMessage& message, int *pExpires)
{
	if (PhoneIp().empty())
	{
		ReportError(NULL, "Sip device is not supported due to lack of dual ip addresses for the host.");
		return;
	}
...
void MtSipStackRuntime::OnStopStack(Sip::MtSipMessage& message)
{
	StopStack();
}

void MtSipStackRuntime::StopStack()
{
	if (m_pDum)
	{
...
void MtSipStackRuntime::onDumCanBeDeleted()
{
//	delete m_pDum;
//	m_pDum = NULL;
    MtSipMessage* msg = new MtSipMessage(Msgs::ClearStack);
    AddMessage(msg);
}

void MtSipStackRuntime::OnClearStack(MtSipMessage& msg)
{
...
void MtSipStackRuntime::ExtractAndSetCallId(Sip::MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
bool MtSipStackRuntime::IsValidCall(const Sip::MtSipMessage& msg)
{
...
void MtSipStackRuntime::OnIncomingCall(Sip::MtSipMessage& msg)
{ 
/*    MtSipIncomingCallMsg* msg = dynamic_cast<MtSipIncomingCallMsg*>(&(MtSipMessage&)msg);
    ACE_ASSERT(msg != 0);
...
void MtSipStackRuntime::OnCallCleared(Sip::MtSipMessage& msg)
{
...
void MtSipStackRuntime::OnGotDigits(Sip::MtSipMessage& msg)
{
...
void MtSipStackRuntime::OnGotCapabilities(Sip::MtSipMessage& msg)
{
/*
if(IsValidCall(msg) == false) return;
...
void MtSipStackRuntime::OnHandleStackMsgDefault(Sip::MtSipMessage& msg)
{
/*
if(IsValidCall(msg) == false) return;
...
void MtSipStackRuntime::OnSetMedia(MtSipMessage& msg)
{
...
void MtSipStackRuntime::OnAnswer(MtSipMessage& msg)
{
...
bool MtSipStackRuntime::CompareMediaOption(const SdpContents* psdp1, const SdpContents* psdp2)
{
...
void MtSipStackRuntime::OnReject(Sip::MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnHangup(Sip::MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::DecodeMediaCaps(FlatMapReader& reader, resip::SdpContents::Session::Medium &m)
{
	//add codecs to the media
	ostringstream os;
	int len;
...
SdpContents* MtSipStackRuntime::BuildSdpForMakeCall(FlatMapReader& reader, int* pCallId, NameAddr* pnaDev)
{
	ostringstream os;
...
void MtSipStackRuntime::OnMakeCall(Sip::MtSipMessage& msg)
{
	ostringstream os;
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnHold(MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnResume(MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnUseMohMedia(MtSipMessage& msg)
{
...
void MtSipStackRuntime::OnBlindTransfer(MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnRedirect(MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnSendUserInput(MtSipMessage& msg)
{
	FlatMapReader reader(msg.Payload());
...
void MtSipStackRuntime::OnHandleAppServerMsgDefault(Sip::MtSipMessage& msg)
{
...
void MtSipStackRuntime::SendCallEstablished(int cid)
{
...
void MtSipStackRuntime::SendDigits(int cid, const string& digits)
{
	auto_ptr<FlatMapWriter> pfmw(new FlatMapWriter());
	//callid
	pfmw->insert(Params::CallId, cid);
...
MtSipCallState* MtSipStackRuntime::GetCallStateByID(const char* pCallId)
{
/*
...
void MtSipStackRuntime::ReportError(int callId, const char* pszMsg)
{
...
InviteSessionHandle MtSipStackRuntime::FindInviteSession(FlatMapReader& reader)
{
...
NameAddr *MtSipStackRuntime::ReadAndParseNameAddr(FlatMapReader& reader, int key, int callId)
{
	ostringstream os;
...
void MtSipStackRuntime::SetRegistrationPortRange(int minPort, int maxPort)
{
...
void MtSipStackRuntime::UpdateRegistrationPortRange(int minPort, int maxPort)
{
	m_minPortMutex.acquire();
...
int MtSipStackRuntime::NextFreePort()
{
	int port;
...
int MtSipStackRuntime::MaxPort()
{
	return m_maxPort;
}

void MtSipStackRuntime::UpdateDevicePortMap(const char* device, int port, SipDevice::Status s)
{
...
void MtSipStackRuntime::UpdateDeviceStatus(const char* device, ClientRegistrationHandle h, SipDevice::Status s)
{
	m_devicePortMapMutex.acquire();
	SipDevice *pd = m_devicePortMap[device];
	if ( pd == NULL)
...
SipDevice::Status MtSipStackRuntime::DeviceStatus(const char* device)
{
...
ClientRegistrationHandle MtSipStackRuntime::RegistrationHandleForDevice(const char* device)
{
...
int MtSipStackRuntime::PortForDevice(const char* device)
{
	int port = 0;
	m_devicePortMapMutex.acquire();
...
int MtSipStackRuntime::RemovePortForDevice(const char* device)
{
	int port = 0;
	m_devicePortMapMutex.acquire();
...
bool MtSipStackRuntime::InitializeIpForPhones()
{
	bool rc = false;
	...
	{
		SIPLOG((Log_Info, "There are no dual IP addresses defined for the host. Only trunk interface will be enabled."));
		rc = false;
	}
	else
	{
...
MtSipStackRuntime::getInterfaces(const Data& matching)
{
...
string MtSipStackRuntime::PhoneIp()
{
	return m_phoneIp;
}

Log::Level MtSipStackRuntime::TranslateToResipLogLevel(int level)
{
