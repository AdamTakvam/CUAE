...
{
    /// <summary>
    /// Increments participant count for a conference
    /// </summary>
    [PackageDecl("Metreos.Native.ScheduledConference")]
    public class IncrementParticipantCount : INativeAction
    {
        ...
        public IncrementParticipantCount()
        {
            Clear();
        }
        ...
        [Action("IncrementParticipantCount", false, "Increments the participant count for a conference", "Increments the participant count for a conference")]
        [ReturnValue(typeof(SC.ReturnValues), "Return values of ScheduledConference native actions")]
        public string Execute(SessionData sessionData, IConfigUtility configUtility)
        {
            string sqlString = string.Format(SC.CountUpdateString, "+", Convert.ToString(conferencePin));
            ...
            try
            {
                int numAffectedRows = ScheduledConference.ExecuteNonQuery(sqlString, connection);
                
                if (numAffectedRows == 0)
                {
                    object[] msgArray = new object[2] { "Found no ScheduledConference records for pin: ", conferencePin } ;
                    log.Write(TraceLevel.Warning, "{0}{1}", msgArray);
            ...
            catch (Exception e)
            {
                ...
                log.Write(TraceLevel.Warning, "Error encountered in the IncrementParticipantCount method, using pin: {0}\n"+
                    "Error message: {1}", msgArray);
                return SC.ReturnValues.failure.ToString();
            }
            finally
