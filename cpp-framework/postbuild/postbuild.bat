...
set ProjectRoot=%MetreosWorkspaceRoot%\cpp-framework
set DeployDir=%MetreosBuildRoot%\Framework\1.0
...
@echo BUILDTARGET=%BuildTarget% >> %PostBuildLog%
...
xcopy %ProjectRoot%\lib\*.dll   %DeployDir% /Y				>> %PostBuildLog%
xcopy %ProjectRoot%\lib\*.lib   %DeployDir% /Y				>> %PostBuildLog%
xcopy %ProjectRoot%\lib\*.pdb   %SymbolDir% /Y				>> %PostBuildLog%

rem -- Install debg symbols
