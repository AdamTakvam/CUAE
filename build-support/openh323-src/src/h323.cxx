...
#ifdef DEFER_OLC
  deferredOpenLogicalChannels.AllowDeleteObjects();
  deferredOpenLogicalChannels.RemoveAll();
#endif
...
BOOL H323Connection::WriteControlPDU(const H323ControlPDU & pdu)
{
  ...
  // LRM 12/4/2005: Commented out as the call to GetLocalAddress() appears to crash under load.
  //
...
#ifdef DEFER_OLC
void H323Connection::AddDeferredOpenLogicalChannel(const H245_OpenLogicalChannel & pdu)
{
  const H245_OpenLogicalChannel_forwardLogicalChannelParameters_multiplexParameters & muxParms =
    pdu.m_forwardLogicalChannelParameters.m_multiplexParameters;
  if (muxParms.GetTag() ==
      H245_OpenLogicalChannel_forwardLogicalChannelParameters_multiplexParameters::e_h2250LogicalChannelParameters) {
    const H245_H2250LogicalChannelParameters & openParms = pdu.m_forwardLogicalChannelParameters.m_multiplexParameters;
    deferredOpenLogicalChannelsMutex.Wait();
    delete UnsafeRemoveDeferredOpenLogicalChannel(openParms.m_sessionID); // Assure only one with this session id
    deferredOpenLogicalChannels.Append(pdu.Clone());
    deferredOpenLogicalChannelsMutex.Signal();
  }
}

const H245_OpenLogicalChannel * H323Connection::RemoveDeferredOpenLogicalChannel(unsigned sessionID)
{
  PWaitAndSignal mutex(deferredOpenLogicalChannelsMutex);
  return UnsafeRemoveDeferredOpenLogicalChannel(sessionID);
}

const H245_OpenLogicalChannel * H323Connection::UnsafeRemoveDeferredOpenLogicalChannel(unsigned sessionID)
{
  const H245_OpenLogicalChannel * selectedPDU = NULL;
  for (PINDEX i = 0; i < deferredOpenLogicalChannels.GetSize(); ++i) {
    const H245_OpenLogicalChannel * pdu = (const H245_OpenLogicalChannel *)deferredOpenLogicalChannels.GetAt(i);
    if (pdu != NULL) {
      const H245_H2250LogicalChannelParameters & openParms = pdu->m_forwardLogicalChannelParameters.m_multiplexParameters;
      if (openParms.m_sessionID == sessionID) {
        deferredOpenLogicalChannels.DisallowDeleteObjects();
        const H245_OpenLogicalChannel * removedPdu =
          (const H245_OpenLogicalChannel *)deferredOpenLogicalChannels.RemoveAt(i);
        PAssert(removedPdu == pdu, PLogicError);
        selectedPDU = pdu;
        break;
      }
    }
