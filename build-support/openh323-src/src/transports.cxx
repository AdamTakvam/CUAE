...
void H323ListenerTCP::Main()
{
  ...
  while (listener.IsOpen()) {
    try {
      ...
      PTRACE(1, "H323\texception raised during socket accept, about the refresh H323 stack.");
      closedDueToError = TRUE;
      break;
    }
  }

  // JDL, 12/12/05, Looks like the socket error may be caused by reasons other than Accept
  // failures.  Open up the recovering callback to handle all socket errors.
  //if (closedDueToError)
  //endpoint.OnListenerClosed(this, closedDueToError);
  endpoint.OnListenerClosed(this, TRUE);
}
...
void H323TransportIP::SetUpTransportPDU(H225_TransportAddress & pdu, BOOL localTsap) const
{
  ...
  if (!localTsap) 
    ...
    transAddr = H323TransportAddress(ipAddr, localPort);
  }
