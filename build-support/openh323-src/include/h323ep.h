...
class H323EndPoint : public PObject
{
...
#ifdef H323_TRANSNEXUS_OSP
  /**@name OSP server management */
  //@{
    /**Add an OSP provider to the endpoint
       This allows for the lookup of destinations via an OSP server. Note that there can be
       only one OSP provider per endpoint, so any existing provider will be destroyed
      */
    inline int SetOSPProvider(
      const PString & server
    )
    { return SetOSPProvider(server, PDirectory()); }
    virtual int SetOSPProvider(
      const PString & server,
      const PDirectory & ospDir
    );
    virtual void SetOSPProvider(
      OpalOSP::Provider * provider
    );

    /** Get the current OSP provider (if any)
      */
    virtual OpalOSP::Provider * GetOSPProvider()
    { return ospProvider; }
  ...
  //@{
    ...
    /**Callback invoked when an H323Listener is closed.
       Default behavior is to do nothing.
      */
    virtual void OnListenerClosed(
      H323Listener* listener, /// Listener that has been closed
      BOOL closedDueToError   /// Did the listener close because of an error condition?
    );
  ...
  //@{
    ...
    /** Set a new timeout for connecting the H.225 TCP socket
        with the remote endpoint.
     */
    void SetSignallingChannelConnectTimeout(PTimeInterval newConnectTimeout) { signallingChannelConnectTimeout = newConnectTimeout; }
