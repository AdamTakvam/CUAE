...
echo ** Executing Unit Tests          >> %UnitTestLog%
%CUAEWORKSPACE%\Tools\NUnit-2.2.9\bin\nunit-console.exe %ProjectRoot%\UnitTest\bin\Debug\UnitTest.dll /config=%ProjectRoot%\UnitTest\bin\Debug\UnitTest.dll.config   >> %UnitTestLog%

rem ** Execute Unit Tests
rem echo UNITTEST: MODULE-NAME.StubTest.Stub01          >> %UnitTestLog%
rem echo stub unit test, replace with something real    >> %UnitTestLog%
rem echo RESULT: PASS                                   >> %UnitTestLog%
rem echo UNITTESTCOMPLETE: 1 PASS, 0 FAIL, 0 SKIPPED    >> %UnitTestLog%
