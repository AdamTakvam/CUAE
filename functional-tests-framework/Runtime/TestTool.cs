...
namespace Metreos.Samoa.FunctionalTestRuntime
{
	...
	public class TestTool : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(TestTool));
		...
		public TestTool(IFunctionalTestConduit conduit)
		{
			//
			...
			conduit.Output += new CommonTypes.OutputLine(OutputLine);
            ...
            Logger.Instance.verboseMessageSink += new LoggerWriteDelegate(LogOutput);
            Logger.Instance.infoMessageSink += new LoggerWriteDelegate(LogOutput);
            Logger.Instance.warningMessageSink += new LoggerWriteDelegate(LogOutput);
            Logger.Instance.errorMessageSink += new LoggerWriteDelegate(LogOutput);
		...
		private void StatusUpdate(string status)
		{
			...
			{
				this.status = status;
				this.statusBar1.Text = connected ? "Connected to " + String.Join(", ", conduit.Settings.AppServerIps) + ":  "  + status : "Not Connected:  " + status;
			}
			else
				this.statusBar1.Text = connected ? "Connected to " + String.Join(", ", conduit.Settings.AppServerIps)  + ":  "  + this.status  : "Not Connected:  " + this.status;
		}
        ...
        private void AddNode(int baseNameSpaceLength, string fullTestName, ArrayList inputData, Hashtable configValues, bool firstTime, bool previousSuccess, string description, string instructions)
        {
            ...
            foreach(TestInputDataBase input in inputData)
            {
                ...
                input.FullTestName = fullTestName;
            }
        ...
        private void menuItem16_Click(object sender, System.EventArgs e)
        {
            ...
            FileInfo info = new FileInfo(conduit.TestSettingsFolder + Path.DirectorySeparatorChar + conduit.Settings.TestSettings);
        ...
        private void menuItem15_Click(object sender, System.EventArgs e)
        {
            ...
            FileInfo info = new FileInfo(conduit.TestSettingsFolder + Path.DirectorySeparatorChar + "Settings.txt");
        ...
        {
            OutputLine("Connecting to Application Server at " + String.Join(", ", conduit.Settings.AppServerIps) + ':' + conduit.Settings.SamoaPort);

            conduit.ConnectServer.BeginInvoke(conduit.Settings.Username, conduit.Settings.Password, new System.AsyncCallback(ConnectDone), null);
        }
...
//			}
		...
		private void SaveTestSettings(object testSettingsFileName)
		{
			...
			lock(testSettingsFileLock)
			{
				try
				{
					FileInfo fileInfo = new FileInfo(conduit.TestSettingsFolder + Path.DirectorySeparatorChar + testSettingsFileName as string);
        ...
        private void LogOutput(DateTime timeStamp, System.Diagnostics.TraceLevel errorLevel, string message)
        {
            OutputLine(errorLevel.ToString()[0] + " " + message); 
        }
    }
}
