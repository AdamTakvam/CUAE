...
namespace Metreos.Samoa.FunctionalTestFramework
{
	...
	/// </summary>
	public class AppIntervalResultsCreation : TestLogCreationBase
	{
		...
		public AppIntervalResultsCreation(StringCollection info, string title, StringCollection times, string units, int interval, int numberOfSignalsSent) : base(info, title)
		{
            if(info == null)
            {
        ...
        {
            XHTMLBuilder = new StringBuilder("<div class='appInterval'><span class='title'>" + title + "</span>");
            XHTMLBuilder.Append("<p><span class='info'>The amount of time used between sending \"trigger app\" signals: <span>" + interval + " " + units + "</span></span></p>");
            ...
            {
                XHTMLBuilder.Append("<p><span class='warning'>All \"app shutdown\" signals were not received!</span><br/>");
            ...
            if(info.Count != 0)
            {
                XHTMLBuilder.Append("<table border='0' class='appInterval'>");
                XHTMLBuilder.Append("<th class='appInterval'>Received \"Shutdown Signal\" Number</th><th class='appInterval'>" + units + "&nbspSince Last Signal</th><th class='appInterval'>Time Received</th>");
                ...
                for(int i = 0; i < info.Count; i++)
                {
                    ...
                    if(evenOrOdd == 0)
                    {
                        XHTMLBuilder.Append("<tr class='appInterval'><td class='even'>" + i + "</td><td class='even'>" + info[i] + "</td><td class='even'>" + times[i] + "</td></tr>");
                    }
                    else
                    {
