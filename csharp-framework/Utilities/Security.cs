...
namespace Metreos.Utilities
{
    ...
    {
        /// <summary>Perform a one-way MD5 encryption on the specified string</summary>
        /// <param name="password">Any string value</param>
        /// <returns>The MD5 hash of the input string</returns>
        public static string EncryptPassword(string password)
        {
            MD5CryptoServiceProvider crypto = new MD5CryptoServiceProvider();
            byte[] md5Bytes = crypto.ComputeHash(System.Text.Encoding.Default.GetBytes(password));
            return System.Convert.ToBase64String(md5Bytes);
        }

        public class Aes
        {
            /// <summary>
            ///     Encrypts a string using AES.  Does not support trailing whitespace.  Front-loaded whitespace is fine.
            /// </summary>
            /// <param name="unencrypted">Unencrypted content</param>
            /// <param name="key">8 characters, 12 characters, or 16 characters</param>
            /// <returns>Base64-encoded encrypted content</returns>
            public static string EncryptB64(string unencrypted, string key)
            {
                byte[] encBytes = Encrypt(unencrypted, key);
                return encBytes == null ? null : Convert.ToBase64String(encBytes);
            }
            ...
            public static byte[] Encrypt(string unencrypted, string key)
            {
                if(unencrypted == null || unencrypted == String.Empty) return null;

                if(key.Length != 8 && key.Length != 12 && key.Length != 16)
                {
                    throw new ArgumentException("Key must be of 8, 12, or 16 characters", "key");
                }

                AesFramework.KeySize keySize;
                
                if(key.Length == 8)
                {
                    keySize = AesFramework.KeySize.Bits128;
                }
                else if(key.Length == 12)
                {
                    keySize = AesFramework.KeySize.Bits192;
                }
                else
                {
                    keySize = AesFramework.KeySize.Bits256;
                }
                ...
                byte[] keyBytes = new byte[chunkSize];  
                ...
                do
                {
                    if(unencrypted.Length * 2 - offset >= chunkSize)
                    {
                        plainText = Encoding.Unicode.GetBytes(unencrypted.Substring(offset / 2, chunkSize / 2));
                        AesFramework aes = new AesFramework(keySize, keyBytes);
                        aes.Cipher(plainText, cipherText);
                        encryptedString.Append(Encoding.Unicode.GetString(cipherText));
                    }
                    else
                    {
                        lastChunk = true;
                ...
                if(unencrypted.Length * 2 - offset > 0) // We didn't fall right on the end of the string, or beyond
                {                    
                    plainText = Encoding.Unicode.GetBytes(unencrypted.Substring(offset / 2).PadRight(chunkSize / 2, ' '));
                    AesFramework aes = new AesFramework(keySize, keyBytes);
                    aes.Cipher(plainText, cipherText);
                    encryptedString.Append(Encoding.Unicode.GetString(cipherText));
                }

                return System.Text.Encoding.Unicode.GetBytes(encryptedString.ToString());
            }


            /// <summary>
            ///     Decrypts a string using AES.  Be sure to use the same key as when encrypting.  Trailing whitespace in the decrypted string will be removed
            /// </summary>
            /// <param name="encrypted">Base64-encoded encrypted content</param>
            /// <param name="key">8 characters, 12 characters, or 16 characters</param>
            /// <returns>Decrypted content</returns>
            public static string DecryptB64(string encrypted, string key)
            {
            ...
            public static string Decrypt(byte[] encryptedBytes, string key)
            {
                ...
                byte[] cipherText       = new byte[chunkSize];
                byte[] decipheredText   = new byte[chunkSize]; 
                byte[] keyBytes         = new byte[chunkSize];     
                ...
                do
                {
                    if(encrypted.Length * 2 - offset >= chunkSize)
                    {
                        cipherText = Encoding.Unicode.GetBytes(encrypted.Substring(offset / 2, chunkSize / 2));
                        AesFramework aes = new AesFramework(keySize, keyBytes);
                        aes.InvCipher(cipherText, decipheredText);
                        unencryptedString.Append(Encoding.Unicode.GetString(decipheredText));
                    }
                    else
                    {
                        lastChunk = true;
                    ...
                    Array.Clear(cipherText, 0, cipherText.Length);
                    Array.Clear(decipheredText, 0, decipheredText.Length);
                ...
                if(encrypted.Length * 2 - offset > 0) // We didn't fall right on the end of the string, or beyond
                {  
                    cipherText = Encoding.Unicode.GetBytes(encrypted.Substring(offset / 2).PadRight(chunkSize / 2, ' '));
                    AesFramework aes = new AesFramework(keySize, keyBytes);
                    aes.InvCipher(cipherText, decipheredText);
                    unencryptedString.Append(Encoding.Unicode.GetString(decipheredText));
                }

                return unencryptedString.ToString().TrimEnd(' ');
            }
        }

        // Taken from MS Samples.  License states that distro is fine as long as you don't hold MS accountable, 
        // or put their name or logo on this work. Basically, they don't want to be liable.

        protected class AesFramework  // Advanced Encryption Standard
        {
            ...
            public AesFramework(KeySize keySize, byte[] keyBytes)
            {
                SetNbNkNr(keySize);
