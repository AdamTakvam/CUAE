...
using RecordTest = Metreos.TestBank.Provider.Provider.RecordDigit;
...
namespace Metreos.FunctionalTests.SingleProvider.MediaServer
{
    ...
    public class RecordDigit : FunctionalTestBase
    {
        ...
        public RecordDigit() : base(typeof( RecordDigit ))
        {
        }
        ...
        public override bool Execute()
        {
            ...
            if(routingGuid == null)
            {
                log.Write(System.Diagnostics.TraceLevel.Info, "The routing guid for the script was unretreivable.");
                return false;
            }
            ...
            SendEvent(RecordTest.script1.E_SendDigit.FullName, routingGuid);

            if( !WaitForSignal( RecordTest.script1.S_FirstPlayDone.FullName, playFileLength + 10) )
            {
