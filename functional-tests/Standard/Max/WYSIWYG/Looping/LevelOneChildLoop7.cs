...
using LevelOneChildLoopTest = Metreos.TestBank.Max.Max.LevelOneChildLoop7;
...
namespace Metreos.FunctionalTests.Standard.Max.WYSIWYG.Looping
{
	...
	public class LevelOneChildLoop7 : FunctionalTestBase
	{
        ...
        public LevelOneChildLoop7() : base(typeof( LevelOneChildLoop7 ))
        {
        ...
        public override bool Execute()
        {
            ...
            for(int i = 0; i < levelZeroCount; i++)
            {
                for(int j = 0; j < levelOneCount; j++)
                {
                    ...
                    if(typeOneCount - 1 != (i * levelOneCount + j))
                    {
                    ...
                    {
                        log.Write(System.Diagnostics.TraceLevel.Info, "Did not the signal from the level one child loop " + LevelOneChildLoopTest.script1.S_LevelOneB.Name);
                        return false;
                    }

                    if(typeTwoCount - 1 != (i * levelOneCount + j))
                    {
                        log.Write(System.Diagnostics.TraceLevel.Info, "First loop did not execute the proper number of times.");
                        return false;
                    }
