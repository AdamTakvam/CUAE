...
namespace CallGenerator
{
	...
	class Class1
	{
		...
		static void Main(string[] args)
		{
            ...
            startTo = int.Parse(args[1]);
            callRange = int.Parse(args[2]);
            int bhca = int.Parse(args[3]);

            baseUrl = String.Format("http://{0}:8000/MakeCall?to=", appServerIp);
        ...
        private static void GotResponse(bool success, string body)
        {
            ...
            if (success)
            {
                ...
                {
                    failedCalls++;
                    System.Console.WriteLine("The call did not complete!");
        ...
        {
            string to = (startTo + counter++ % callRange).ToString();
            return baseUrl + to;
        }
    }
}
