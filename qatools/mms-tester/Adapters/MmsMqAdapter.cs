...
namespace Metreos.MmsTester.Custom.Adapters
{
    ...
    public class MmsMqAdapter : AdapterBase
    {
        ...
        protected Hashtable pendingResponseTypes;
        protected Hashtable PendingResponseTypes;
		...
		public MmsMqAdapter(string taskName, string logLevel) : base(taskName, logLevel)
		{
            ...
            pendingResponseTypes = new Hashtable();
            PendingResponseTypes = Hashtable.Synchronized(pendingResponseTypes);
            
            string queueId = "metreos-provider.mediaserver";
            taskQueue = new MessageQueue(new MsmqMessageQueueProvider(queueId));
        ...
        public override bool Initialize(string mediaServerHandle)
        {
            ResourceProvider.PopMediaServer(mediaServerHandle, out mediaServerMachineName, out mediaServerMessageQueueName);
        ...
        public override bool Send(InternalMessage im, IAdapterTypes.ResponseFromMediaServerDelegate incomingResponseCallback, string messageType)
        {   
            PushPendingRequest(im, incomingResponseCallback, messageType);
        ...
        {
            IAdapterTypes.ResponseFromMediaServerDelegate incomingResponseCallback;
            string messageType;

            if(PopPendingRequest( message, out incomingResponseCallback, out messageType))
            {
                InternalMessage im = incomingResponseCallback(message, messageType);
            }
            else
            {
                return false;
        ...
        private bool PopPendingRequest( InternalMessage im, out IAdapterTypes.ResponseFromMediaServerDelegate incomingResponseCallback, out string messageType)
        {
            messageType = null;
            ...
            {
                incomingResponseCallback = (IAdapterTypes.ResponseFromMediaServerDelegate) PendingResponses[transactionId];
        ...
        private bool PushPendingRequest(InternalMessage im, IAdapterTypes.ResponseFromMediaServerDelegate incomingResponseCallback, string messageType)
        {
            string transactionId;
            im.GetFieldByName(MmsProtocol.FIELD_MS_TRANSACTION_ID, out transactionId);
            ...
            if(!PendingResponses.Contains(transactionId))
            {
                ...
                if(!PendingResponseTypes.Contains(transactionId))
                {
                    PendingResponseTypes[transactionId] = messageType;
                    return true;
                }
                else
                {
        ...
        // Unimplemented
        public override bool Restart(string mediaServerHandle)
        {
            return false;
        }
	}
