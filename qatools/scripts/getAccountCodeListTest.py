...
## Test for the getAccountCode AppsuiteAdmin WebServices API
...
##  TESTCASE-AS-SOAP-WS-API-0401 - getAccountCodeList. Users exist - PASS
    # create 3 users and get them
    test = 0
    failure = 0
    initUserRecord(userRecord)
    expected = 'PASS'
    testcase = 'TESTCASE-AS-SOAP-WS-API-0401 - getAccountCodeList. Users exist' 
    ...
    if failure == 0:
        try:
            users = s.getAccountCodeList(token)
            
            userTest = len(users) > 0
            usernameCode = 0
            for user in users:
                userTest &= user == str((numTestCases + usernameCode))
...
##  TESTCASE-AS-SOAP-WS-API-0403 - getAccountCodeList. Invalid token - FAIL
    # attempt getAccountCodeList with bad token
    test = 0
    expected = 'FAIL'
    testcase = 'TESTCASE-AS-SOAP-WS-API-0403 - getAccountCodeList. Invalid token' 

    try:
        users = s.getAccountCodeList('bleh')
        test = 1
    except SOAPpy.Types.faultType, err:
            #print '\nFAILED: %s. Unexpected exception %s' % (testcase, err.faultstring)
            pass
...
##  TESTCASE-AS-SOAP-WS-API-0402 - getAccountCodeList No users exist - PASS
    # deleteAllUsers, then perform getAccountCodeList
    test = 0
    expected = 'PASS'
    testcase = 'TESTCASE-AS-SOAP-WS-API-0402 - getAccountCodeList. No users exist' 
        
    try:
        s.deleteAllUsers(token)
        users = s.getAccountCodeList(token)
        test = users == None or len(users) == 0
    except SOAPpy.Types.faultType, err:
            print '\nFAILED: %s. Unexpected exception %s' % (testcase, err.faultstring)
            pass
...
def main(*argv): # expects foldername
    ...
    try:
        try:
            ...
            print "\nStarting getAccountCode test suite against %s\n" % args[0]
