...
namespace devicecreator
{
	...
	public class Worker
	{
        ...
        public bool Generate(   long deviceStart, int baseDn, int numPhones, 
                                ...
                                string partition, string phoneTemplateName, long deviceProfileStart)
        {
            for(int i = 0; i < numPhones; i++)
            {
               string lineId = null;
                ...
                else
                {
                    ...
                    ReportError(String.Format("Phone {0} will not have a line.", ConvertMACToSEP(deviceStart + i)));
                }
                ...
                {
                    xipphone.enableExtensionMobility = true;
                    xipphone.Item10 = ConvertMACToDP(deviceProfileStart + i);
                }
                ...
                try
                {
                    ...
                    Console.WriteLine("{0} added with line number {1}", ConvertMACToSEP(deviceStart + i), lineId == null ? "-" : (i * increment + baseDn).ToString());
                }
                catch(Exception e)
                {
                    ReportError(String.Format("Unable to add the phone '{0}'.  \n{1}", ConvertMACToSEP(deviceStart + i), GetDetail(e)));
                }
